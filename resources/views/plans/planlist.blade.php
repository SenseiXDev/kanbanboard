        ...
        @if ($plan->status)
            <div class="col-xs-12 col-sm-12 col-md-{{ $size }} col-lg-{{ $size }} mt-4">
                <div class="card card-pricing popular text-center px-3 mb-5 mb-lg-0">
                    <span
                        class="h6 w-60 mx-auto px-4 py-1 rounded-bottom bg-primary text-white">{{ $plan->name }}
                    </span>
                    ...
                    <div class="card-body delimiter-top">
                        <ul class="list-unstyled mb-4">
                            @if ($plan->id != 1)
                                ...
                                @endif
                                <li>{{ __('Monthly Price') }}:
                                    {{ env('CURRENCY') ? env('CURRENCY') : '$' }}{{ $plan->monthly_price }}</li>
                                <li>{{ __('Annual Price') }}:
                                    {{ env('CURRENCY') ? env('CURRENCY') : '$' }}{{ $plan->annual_price }}</li>
                            @endif
                            @if ($plan->max_users != 0)
                                <li>{{ $plan->max_users < 0 ? __('Unlimited') : $plan->max_users }} {{ __('Users') }}
                                </li>
                            @endif
                    ...
                    <div class="card-footer delimiter-top">
                        <div class="row justify-content-center">
                            <?php $planStatus = true; ?>
                            @if (\Auth::user()->type != 'client' && \Auth::user()->plan != '' && \Auth::user()->plan == $plan->id &&
                            ...
                            @elseif($paymentSetting['enable_stripe'] == 'on' ||
