...
class ProjectReportController extends Controller
{
    ...
    public function index()
    {
            ...
            //   $project_status = Project::whereIn('id', array_keys($user_projects))->groupBy('status')->get();
              $project_status = Project::whereIn('id', array_keys($user_projects))->get();
    ...
    public function ajax_data(Request $request)
    {
         ...
         if ($request->title) {
            $projects->where('title', '=', $request->title);
        }
        ...
        foreach($projects as $project) {
            ...
            $tmp['id'] = $project->id;
            // $tmp['name'] = $project->title;


                if($project->is_active == 1){
                    $tmp['name'] =
                        '<a  class="name mb-0 h6 text-sm" data-toggle="popover"  title="' . __('view Project') . '" data-size="lg" data-title="' . __('show') . '" href="' . route('projects.show',$project->id) . '">'.$project->title.'</a>';
                        }
                        else{
                            $tmp['name'] ='<a>'.$project->title.'</a>';
                        }
...
public function show(Request $request ,$id)
{
    $objUser = Auth::user();
        ...
        if ($project) {
            ...
            $stages = TaskStage::where('created_by', '=', $objUser->id)->where('project_id',$id)->get();
    ...
    public function ajax_tasks_report(Request $request ,$id)
    {
        ...
        $usr_contact = UserContact::where('user_id', $userObj->id)->where('role','user')->orWhere('role','client')->first();
        ...
        foreach($timesheets_task as $timesheet)
        {
            ...
            $tmp['title'] = '<a href="' . route(
                'projects.tasks.index', $task->project_id) . '" class="text-body">' . $task->title . '</a>';
