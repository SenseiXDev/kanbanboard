...
class HomeController extends Controller
{
    ...
    public function index()
    {
        ...
        else
        {
            ...
            $user_plan = Plan::find($users->plan);
            if($user_plan->storage_limit > 0)
            {
                $storage_limit = ($users->storage_limit / $user_plan->storage_limit) * 100;
            ...
            return view('admin.dashboard', compact('home_data','users','user_plan','storage_limit'));
        }
    }
    ...
    {
        $usr   = Auth::user();
        $users = User::where('id', '!=', $usr->id)->where('type','Owner');
