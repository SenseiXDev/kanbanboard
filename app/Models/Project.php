...
class Project extends Model
{
    ...
    public function getImgImageAttribute()
    {
        ...
        if(\File::exists($this->image) && !empty($this->image))
        {
            //return $this->attributes['img_image'] = 'src='.asset(\File::url($this->image));
            return $this->attributes['img_image'] = 'src='.$logo.$this->image;
        }
        else
        {
    ...
    public function permission()
    {
        ...
        $permissions = $this->users()->where('user_id', $user_id)->first();
        if(!empty($permissions))
        {
            $permissions = $permissions->pivot->permission;
            return $permissions;
        }
    ...
    public function projectAttachments($id = null)
    {
        if($id == null){
            $usr  = User::find($this->created_by);
    ...
    public static function getProjectAssignedTimesheetHTML($projects_timesheet = null, $timesheets = [], $days = [], $project_id = null, $seeAsOwner = false)
    {
        ...
        if($seeAsOwner)
        {
            ...
            if($project_id == '0')
            {
                ...
                foreach($timesheets as $project_id => $timesheet)
                {
                    ...
                    if($project)
                    {
                        ...
                        $timesheetArray[$k]['project_name'] = $project->title;
            ...
            else
            {
                foreach($timesheets as $task_id => $timesheet)
                {
                    ...
                    $task = ProjectTask::find($timesheet['task_id']);
                    if($task)
                    {
            ...
            else
            {
                ...
                foreach($timesheets as $task_id => $timesheet)
                {
                    ...
                    if($task)
                    {
                        ...
                        {
                            $date                                        = $days['datePeriod'][$j]->format('Y-m-d');
                            $key                                         = $date == $timesheet['date'];
                            $timesheetArray[$i]['dateArray'][$j]['date'] = $date;

                            if($key !== false)
                            {
                                $time                                        = Carbon::parse($timesheet['time'])->format('H:i');
                                $times[]                                     = $time;
        ...
        $returnHTML = view('projects.timesheets.week', compact('timesheetArray', 'totalDateTimes', 'calculatedtotaltaskdatetime', 'days', 'seeAsOwner', 'allProjects'))->render();
